...
{
    interval_queue = xQueueCreate(QUEUE_LENGTH, sizeof(uint32_t));
    ...
    while (1)
    {
        ...
        switch (state)
        {
            case SENSOR_OFF:
                ...
                if (now - state_timer >= SENSOR_OFF_TIME_SEC * 1000000LL)
                {
                    state = SENSOR_WARMUP;
            ...
            case SENSOR_WARMUP:
                ...
                if (now - state_timer >= SENSOR_WARMUP_TIME_SEC * 1000000LL)
                {
            ...
            case SENSOR_MEASURE:
                ...
                if (now - state_timer >=
                    (SENSOR_ON_TIME_SEC - SENSOR_WARMUP_TIME_SEC) * 1000000LL)
                {
                    state = SENSOR_OFF;
        ...
        if (state != SENSOR_OFF)
        {
            if (xQueueReceive(interval_queue, &interval, pdMS_TO_TICKS(50)))
            {
        ...
        if (now - last_send >= SEND_INTERVAL_MS * 1000)
        {
            ...
            {
                tds = calculate_tds(filtered_freq, flow_rate_lmin);
